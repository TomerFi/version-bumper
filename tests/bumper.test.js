...
suite('Test automatic bumps', () => {
    ...
    test(`testing with a repository with no tags
        ...
        return expect(bumperSut({source: 'git', bump: 'auto', label: '-alpha1', preset: 'angular', path: noTags}))
            .to.eventually.deep.equal({original: 'none', bump: 'none', next: '1.0.0', dev: '1.0.0-alpha1'})
    })
    ...
    test(`testing with a repository with no new commits
        ...
        return expect(bumperSut({source: 'git', bump: 'auto', label: '-alpha1', preset: 'angular', path: noCommits}))
            .to.eventually.deep.equal({original: '1.2.3', bump: 'patch', next: '1.2.4', dev: '1.2.4-alpha1'})
    })
    ...
    test(`testing with a feat type commit
        ...
        return expect(bumperSut({source: 'git', bump: 'auto', label: '-alpha1', preset: 'angular', path: minorBump}))
            .to.eventually.deep.equal({original: '1.2.3', bump: 'minor', next: '1.3.0', dev: '1.3.0-alpha1'})
    })
    ...
    test(`testing with a breaking change commit body
        ...
        return expect(bumperSut({source: 'git', bump: 'auto', label: '-alpha1', preset: 'angular', path: minorBump}))
            .to.eventually.deep.equal({original: '1.2.3', bump: 'major', next: '2.0.0', dev: '2.0.0-alpha1'})
    })
})
